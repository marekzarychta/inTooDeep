...
if (currentState != states.DEATH) {
	...
	if (currentState == states.IDLE) {
		...
		if (!place_meeting(x, y, oLadder) && place_meeting(x, y + 1, oLadder) && !downKey && abs(axisY<=0.5)) {
		...
		if (upKey && place_meeting(x, y, oLadder)) {
			currentState = states.LADDERCLIMB;	
		}
		if (!onGround && !place_meeting(x, y, oLadder)) {
			grav = 0.163;
			currentState = states.FALL;	
		}
		if (onGround && !place_meeting(x, y, oLadder)) {
		...
		sprite_index = sprites[states.LADDERIDLE];
		image_speed = 1;
		...
		sprite_index = sprites[states.LADDERCLIMB];
		image_speed = 1;
		...
		sprite_index = sprites[states.DASH];
		...
		sprite_index = sprites[states.DYING];
		image_speed = 1;
		...
		sprite_index = sprites[states.JUMP];
	...
	//zmiany timerow musza byc niezalezne od stanow, zeby np. cooldown dasha sie zmniejszal jak jestes w innym stanie
	...
	y += yspd;
	if (debug_mode) show_debug_message(string(currentState));
} else {
