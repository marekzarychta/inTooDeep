if (gui) {
	...
	if (talk == max_talk) {
		...
		draw_sprite_stretched(sHeartMini, 0, text_x + padding + 14, no_y - 58 - 4 * padding, 20, 20);
		draw_text(text_x + padding + 40, no_y - 48 - 4 * padding, item.name);
		draw_text(text_x + box_w - 50, no_y - 48 - 4 * padding, item._value);
		...
		draw_sprite_stretched(sSelectedBox, 0, text_x + 2 * padding, (choice == 0 ? no_y - 60 - 5 * padding : no_y - padding - 36), box_w - 4 * padding, 24 + 3 * padding);
		draw_sprite_stretched(sKeyE, 0, text_x + box_w - 70, text_y + box_h - 48, 32, 32);
		draw_sprite_stretched(sKeyE, 0, text_x + box_w - 70, text_y + box_h - 48, 32, 32);
		draw_sprite_stretched(sArrow, 0, text_x + box_w - 38, text_y + box_h - 48, 31, 31);
	}
